...
// 4- Update Category -> Admins Only
export const updateCategory = async (req, res) => {
    try{
        const { role } = req.decoded_token;

        if ( role !== "admin") {
            return res.status(403).json({ message: "Only  admins can update a category" });
        }
        const { name, description, image } = req.body;
...
export const deleteCategory = async (req, res) => {
  try {
    const { role } = req.decoded_token;
        ...
        if (role !== "admin") {
            return res.status(403).json({ message: "Only admins can delete a category" });
        }
